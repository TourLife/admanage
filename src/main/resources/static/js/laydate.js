$(function () {
    ...
    $(".data-icon,.laydateInput").on("click", function (e) {
        ...
        if ($(".select-date").css("display") == "none") {
            ...
            if(_this.attr("code-flag") == "start"){
                $(".select-date").css("left", "0px");
                $(".select-date").addClass("start");
            }else{
                $(".select-date").css("left", "130px");
                $(".select-date").addClass("end");
            }
        } else {
            $(".select-date").css("display", "none");
        ...
        var slected = $(".startTime");
        if($(".select-date").hasClass("end")){
            slected = $(".endTime");
        }
        var now = new Date(slected.val().replace(/\s/g,"","")+" 00:00:00");//.format("yyyy-MM-dd HH:mm");
        var currYear = now.getFullYear();
        var currMonth = (now.getMonth() + 1);
        var currDay = now.getDate();
        $("#yearList").val(currYear + "年");
        $("#monthList").val(currMonth + "月");
        $(".tabel-li").each(function(){
            if($(this).text() == currDay){
                if(!$(this).hasClass("showClick") && !$(this).hasClass("nextDay") && !$(this).hasClass("preDays")){
                    $(".showClick").removeClass("showClick");
                    $(this).addClass("showClick");
                }
            }
        });
    });
    ...
    $("#yearList,#monthList").on("change", function (e) {
        ...
        slected.val($("#yearList").val().split("年")[0] + " - " + $("#monthList").val().split("月")[0] + " - " + currDate);
    });
